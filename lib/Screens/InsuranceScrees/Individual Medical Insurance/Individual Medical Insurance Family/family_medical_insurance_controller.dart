...
class FamilyMedicalInsuranceController extends GetxController {
  ...
  RxList<TextEditingController> memberHeightController = <TextEditingController>[TextEditingController()].obs;
  RxList<TextEditingController> memberWeightController = <TextEditingController>[TextEditingController()].obs;
  RxList<String> selectedMemberRelation = [wife].obs;
  ...
  RxList<String> memberSelectedMaritalStatus = [single].obs;
  RxList<OccuptionList> selectOccupationMember = <OccuptionList>[OccuptionList()].obs;
  RxList<OccuptionList> occupationListMember = <OccuptionList>[].obs;
  RxList selectMemberChronicDiseases = [<GetChronicDiseasesList>[]].obs;
  RxList<GetChronicDiseasesList> getChronicDiseasesListMember = <GetChronicDiseasesList>[].obs;
  RxList<String> memberSelectedPreviousOperationsOption = [''].obs;
  RxList<String> memberChronicOption = [''].obs;
  RxList<TextEditingController> memberPreviousOperationDetailsController = <TextEditingController>[TextEditingController()].obs;
  RxList<String> memberSelectedPregnantOption = [''].obs;
  RxList<String> memberSelectMonth = [month1].obs;
  RxList<String> memberSelectedDangerousActivity = [''].obs;
  RxList<GetDangerousActivitiesList> selectMemberDangerousActivity = <GetDangerousActivitiesList>[GetDangerousActivitiesList()].obs;
  RxList<GetDangerousActivitiesList> getDangerousActivitiesMemberList = <GetDangerousActivitiesList>[].obs;

  RxBool isLoadingIdFrontSideMember = false.obs;
  RxBool isLoadingIdBackSideMember = false.obs;
  RxBool isLoadingPersonalPicMember = false.obs;
  RxBool isLoadingOtherDocumentsMember = false.obs;

  RxList<List<String>> selectedIdFrontSideMember = <List<String>>[].obs;
  RxList<List<String>> selectedIdBackSideMember = <List<String>>[].obs;
  ...
  String? selectmonth;
  String? selectedPregnantOptionMember;
  String? selectmonthMember;
  ...
  RxBool isLoadingOtherDocuments = false.obs;
  ...
  RxList<String> selectedOtherDocuments = <String>[].obs;
  ...
  getInsuranceCurrent(context) async {
    try {
      await adminBasicAllApiController.getInsuranceCurrentApi(context, 7);
  ...
  clearData() {
    ...
    memberSelectedMaritalStatus.clear();
    selectOccupationMember.clear();
    memberHeightController.clear();
    memberWeightController.clear();
    selectMemberChronicDiseases.clear();
    memberSelectedPreviousOperationsOption.clear();
    memberChronicOption.clear();
    memberPreviousOperationDetailsController.clear();
    memberSelectedPregnantOption.clear();
    memberSelectMonth.clear();
    memberSelectedDangerousActivity.clear();
    selectMemberDangerousActivity.clear();
    selectedIdFrontSideMember.clear();
    selectedIdBackSideMember.clear();
    selectedOtherDocumentsMember.clear();
    selectedPersonalPicMember.clear();
    ...
    selectmonth = null;
    selectedDangerousActivity = null;
    ...
    planDd = null;
    isShowHWValidationMsg.value = false;
  }

  showMessage() {
    isShowMessage.value = false;
    for (int i = 0; i < memberSelectedDangerousActivity.length; i++) {
      if (memberSelectedDangerousActivity[i] == 'Yes') {
  ...
  setTextData() {
    ...
    birthDateController.value.text = commonDateFormat(getProfileModelGlobal.data?.birthDate ?? "");
    ...
    try {
      ...
      //countryList.addAll([countryListTemp[countryListTemp.indexWhere((p0) => p0.id == getProfileModelGlobal.data?.countryId)]]);
    ...
    // existingMedicalInsurancePolicyExpiryDateController.value.text = commonDateFormat(DateFormat("yyyy-MM-dd").format(DateTime.parse(DateTime.parse(DateFormat("yyyy-MM-dd").format(initialDate.value)).add(const Duration(days: 364)).toString())));
  }

  getOccupations(context) async {
    try {
      occupationList.clear();
      occupationListMember.clear();
      ...
      occupationListMember.addAll(adminBasicAllApiController.getOccupationModelClass.data ?? []);
    } on DioError catch (e) {
    } catch (f) {}
  }
  ...
  getChronicDiseases(context) async {
    try {
      ...
      getChronicDiseasesListMember.clear();
      ...
      getChronicDiseasesListMember.addAll(adminBasicAllApiController.getChronicDiseaseModelClass.value.data ?? []);
    } on DioError catch (e) {
      isLoading.value = false;
  ...
  getDangerousActivities(context) async {
    try {
      ...
      getDangerousActivitiesMemberList.clear();
      ...
      getDangerousActivitiesMemberList.addAll(adminBasicAllApiController.getDangerousActivitiesModelClass.value.data ?? []);
    } on DioError catch (e) {
      isLoading.value = false;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: AppText(text: e.response!.statusMessage!, txtColor: primaryWhite, size: 12)));
    } catch (f) {
