...
class ApiCall {
  ...
  Future<Map<String, dynamic>> patchRequest({BuildContext? context, required String endpoint, Map<String, dynamic>? body, Options? options}) async {
    try {
      Response response = await dioClient.patch(endpoint, data: body, options: options);
      if (response.data is Map<String, dynamic>) {
        return response.data;
      }
      return {};
    } on DioException catch (e) {
      if (context != null && (e.type == DioExceptionType.connectionError || e.type == DioExceptionType.connectionTimeout || e.error is SocketException)) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: AppText(text: networkError, size: 14, txtColor: primaryWhite)));
      }
      rethrow;
    } catch (e) {
 ...
 /* allApiCall(BuildContext context,Options? options) {
  ...
  void logout(BuildContext context) async {
    ...
    if (g.Get.isRegistered<NotificationController>()) {
      final notif = g.Get.find<NotificationController>();
      notif.detachRtdbListener();
      notif.unreadCount.value = 0;
